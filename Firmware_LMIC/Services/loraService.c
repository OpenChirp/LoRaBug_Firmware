...
#ifndef SERVICES_LORA_C_
#define SERVICES_LORA_C_
...
//static const u1_t DEVEUI[8]  = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10 };
...
void send_periodic( osjob_t* j){
	...
	len = sprintf(send_buff, "Ping %i Times!!!", cnt++);
	...
	switch(ev) {
		...
		case EV_TXCOMPLETE:
			...
			if (LMIC.dataLen) {
				...
				if(strcmp(LMIC.frame[OFF_DAT_OPTS], "Open Sesame")){
			...
			os_setTimedCallback(&sendjob, os_getTime()+ms2osticks(3000), send_periodic);
			break;
